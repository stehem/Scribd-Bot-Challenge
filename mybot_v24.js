...
// init is a mess in need of refactoring
Turn.prototype.init = function(){
  ...
  if (this.board[this.my_x][this.my_y] > 0){
    var on_target = mybot.target !== undefined && mybot.target[0] === this.my_x && mybot.target[1] === this.my_y;
    // if fruit was target then reset
    if (on_target) mybot.target = undefined;
    return TAKE;
  }
  // target was taken or is about to be taken
  var target_taken = mybot.target && (this.board[mybot.target[0]][mybot.target[1]] === 0);
  var opponent_on_target = mybot.target && ( (get_opponent_x() === mybot.target[0]) && (get_opponent_y() === mybot.target[1]) );
  if (target_taken || opponent_on_target) {
    mybot.target = undefined;
    mybot.quad = undefined;
  }
  if (mybot.quad === undefined || !this.has_fruits(mybot.quad) || (this.count_fruits(mybot.quad) < this.count_fruits(this.quad_with_most_types())) ){
...
Turn.prototype.best_to_eat = function(){
  res = [];
  for (i=1;i<6;i++){
    if (get_total_item_count(i) - get_my_item_count(i) - get_opponent_item_count(i) > 0){
...
Turn.prototype.eat_unique = function(){
  var res;
  for (i=1;i<6;i++){
    var left = get_total_item_count(i) - get_opponent_item_count(i) - get_my_item_count(i);
    var diff = get_opponent_item_count(i) - get_my_item_count(i);
    if (left == 1 && (diff === 0 || diff === 1))  res = i;
  }
  if (res !== 'undefined'){
    for (j=0;j<WIDTH;j++){
      for (k=0;k<HEIGHT;k++){
